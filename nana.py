...
#client = LINE()
client = LINE("EtKlxmXtiGJ8vTVEHNd8.T/0Vn2E9Xul8wJjjACMpIa.S6ojMKRPnsRdKLBEElAKQSpI+QXrmeMjHYRLN4AmlzM=")
...
admin = ["ub7ed2f3767bdc542fc5a324969c98f88"]
protectmember = []
...
def helpmessage():
    if settings['setKey'] == True:
        ...
        key = ''
    helpMessage =   "Help Message NayutaBot v2." + "\n" + \
                    "-> " + key + "Help" + "\n" + \
                    "-> " + key + "Translate" + "\n" + \
                    "-> " + key + "TTS" + "\n" + \
                    "->" + " Status Command " +  "\n" + \
...
def selfcom():
    if settings['setKey'] == True:
        ...
        key = ''
    helpSelf =       "Self Command NayutaBot v2." + "\n" + \
                    "-> " + key + "Changepp" + "\n" + \
                    "-> " + key + "Cn: [Text" + "\n" + \
                    "-> " + key + "ChangeBio: [Text]" + "\n" + \
                    "-> " + key + "Me" + "\n" + \
...
def groupcom():
    if settings['setKey'] == True:
        ...
        key = ''
    helpGroup =      "Group Command NayutaBot v2." + "\n" + \
                    "-> " + key + "Gcr" + "\n" + \
                    "-> " + key + "GroupId" + "\n" + \
                    "-> " + key + "GName" + "\n" + \
                    "-> " + key + "GroupPict" + "\n" + \
...
def mediacom():
    if settings['setKey'] == True:
        ...
        key = ''
    helpMedia =      "Media Command NayutaBot v2." + "\n" + \
                    "-> " + key + "CheckDate [Date]" + "\n" + \
...
def settingcom():
    if settings['setKey'] == True:
        ...
        key = ''
    helpSetting =     "Setting Command NayutaBot v2. " + "\n" + \
                     "? AutoAdd [On/Off]" + "\n" + \
                     "? AutoJoin [On/Off]" + "\n" + \
...
def helptexttospeech():
    if settings['setKey'] == True:
        ...
        key = ''
    helpTextToSpeech =  "TextToSpeech Command NayutaBot v2." + "\n" + \
                        "-> " + key + "af : Afrikaans" + "\n" + \
                        "-> " + key + "sq : Albanian" + "\n" + \
                        "-> " + key + "ar : Arabic" + "\n" + \
                        "-> " + key + "hy : Armenian" + "\n" + \
...
def helptranslate():
    if settings['setKey'] == True:
        ...
        key = ''
    helpTranslate = "Translate Command NayutaBot v2." + "\n" + \
                    "-> " + key + "af : afrikaans" + "\n" + \
                    "-> " + key + "sq : albanian" + "\n" + \
                    "-> " + key + "am : amharic" + "\n" + \
                    "-> " + key + "ar : arabic" + "\n" + \
...
def clientBot(op):
    try:
        ...
        if op.type == 5:
            ...
            sendMention(op.param1, "Hi @!, Thanks for Adding me :3")
        ...
        if op.type == 13:
            ...
            if clientMid in op.param3:
                ...
                sendMention(op.param1, "Halo @!, Thanks for Invite me to your group ^^")

        if op.type == 15 and set["leaveMessage"] == True:
        	        sendMention(op.param1, "Goodbye @!, Rest in Peace °?° ", [op.param2])
        ...
        if op.type in [22, 24]:
            ...
            if settings["autoLeave"] == True:
                sendMention(op.param1, "Oi asw @!,ngapain Invite gw Kampank.")
                    ...
                    if msg.contentType == 0:
                        if text is None:
                            ...
                            if cmd == "help":
                                ...
                                helpSetting = settingcom()
                                client.sendMessage(to, str(helpSetting))
                                ...
                                client.sendMessage(to, " Speed send message {} seconds".format(str(elapsed_time)))
                            elif cmd == "runtime":
                                timeNow = time.time()
...
#BATAS SUCI
	                            ...
	                            if msg.to in protectmember: md+="Protect [Active]\n"
                                else: md+="Protect [Non Active ]"
...
# Pembatas Script #
                                ...
                                if 'MENTION' in msg.contentMetadata.keys()!= None:
                                    ...
                                    for ls in lists:
                                        contact = client.getContact(ls)
                                        path  = "http://dl.profile.line-cdn.net/" + contact.pictureStatus + "/vp".format(contact.pictureStatus)
...
# Pembatas Script #   
                                ...
                                try:
                                    ...
                                    r = requests.get("api.dzin.tech/api/instaprofile/?apikey=beta&username".format(search))
                                    data = r.text
                                    data = json.loads(data)
                                    if data != []:
...
#Batas Suci~
                	            ...
                	            try:
                	                ...
                	                #client.sendMessage(to, "Tidak ada id {}".format(str(id)))
                		            client.sendMessage(to, str(wk))
#Batas Suci:-~
...
#__________
                    #        elif cmd.startswith("animestream"):
                   #          	 if sender in admin:
                  #           		  dinn = msg.text.split("|")
                 #           		  result = requests.get("http://moeapi.panel.moe/api/anistream/newpost/?apikey=beta")
                #            		  data = result.txt
            #                		  data = json.loads(data)
             #                		  if len(dinn) == 1:
	          #                  		  num = 0
	      #                      		  ret_ = "??[ Result Anime ]"
	       #                     	      for anime in data["result"]=
	           #                      		        num += 1
	#		        ret_ += "\n? {}. {}".format(str(num), str(anime["Tittle"]))
